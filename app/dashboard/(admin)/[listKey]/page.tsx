...
import { FilterBarWrapper } from "@/components/filter-bar-wrapper";
import { FloatingUIWrapper } from "@/components/ui/floating-ui-wrapper";
import { getListByPath, getAdminMeta } from "@/lib/hooks/getAdminMeta";
import { getListData } from "@/lib/graphql";
...
export default async function ListPage({ params, searchParams }: PageProps) {
  ...
  try {
    ...
    // Fetch the list data using the list metadata
    ...
    return (
      <section aria-label={`${list.label} overview`} className="h-screen overflow-hidden flex flex-col">
        ...
        <div className="flex flex-col flex-1 min-h-0">
          ...
          <FilterBarWrapper list={list} selectedFields={selectedFields} currentSort={sort} />
