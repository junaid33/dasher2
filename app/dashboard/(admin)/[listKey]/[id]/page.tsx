import {
  ...
  updateItem,
  deleteItem,
  getKeystoneAdminMetaWithItemView,
} from "@/lib/graphql";
...
export default async function ItemPage({ params, searchParams }: PageProps) {
  ...
  try {
    ...
    data = await getItem(list, id, searchParamsObj);
    if (!data.item) {
      return <ErrorDisplay title="Item Not Found" message="The requested item could not be found." list={list} />;
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : "An unknown error occurred";
